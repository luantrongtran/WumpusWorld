...
# ---------------------------------------------------------------------------------------------
    ...
    def __init__(self,cave):
        ...
        self.PitWumpus_probability_dis = self.PitWumpus_probability_distribution(self.cave.WIDTH, self.cave.HEIGHT)
        ...
        #Contains all the rooms that are potential for pits/wumpus
        self.query_rooms = set()
...
#---------------------------------------------------------------------------------------------------
    ...
    def has_breeze(self, column, row):
        ...
        if (column,row) in self.visited_rooms or (column==self.current_position[0] and row == self.current_position[1]):
            ...
            tkMessageBox.showinfo("Errors", "The position has not been visited yet.")
            return False
...
#---------------------------------------------------------------------------------------------------
    def observation_pits(self, observed_locations):
        ...
        for each_v in observed_locations:
            ...
            else: tkMessageBox.showinfo("Errors", "The position has not been visited yet.")
        return known_PW
    ...
    def start(self):
        ...
        if self.robot.gameover == False:
            ...
            if self.robot.current_position == self.cave._goldCoor:               
                ...
                tkMessageBox.showinfo("Congrats","You have found the gold and you won!")
            ...
            if self.robot.dead == True:
                ...
                tkMessageBox.showinfo("Game over","You dead!")
            else:
                ## Get the number of moves and set into the result frame
                self.s_move_num.set(str(self.robot.num_moves-1))
                ## Get the result and set into the result frame
                self.s_success.set('No rooms!')
                tkMessageBox.showinfo("Game over","No available rooms to explore, fail!")
    ...
    if b.get() == 1: # Fixed board
        ...
        if (wumpus_column,wumpus_row) in {(1,3),(1,2),(2,3)} or \
           ...
           (gold_row==pit_row and gold_column==pit_column):
            tkMessageBox.showinfo("Wrong setting!",  "The wumpus, pit and gold can't occupy the same room and can't be in the 3 specific rooms. Do it again!")
            return
    else:
        fixed_board = False
...
if __name__ == '__main__':
    ...
    w1 = Spinbox(setting_up, from_=3,to =4)
    ...
    w2= Spinbox(setting_up,from_=3,to =4)
    ...
    w3= Spinbox(setting_up,from_=1,to =3)
